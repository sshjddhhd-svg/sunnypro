    ...
    const intervalId = setInterval(async () => {
      ...
      try {
        await new Promise((resolve) => {
          try {
            const r = currentApi.changeNickname(nickname, threadID, userID, () => resolve());
  ...
  if (type === "تشغيل") {
    ...
    try {
      ...
      const intervalId = setInterval(async () => {
        const currentApi = global._botApi || api;
        if (index >= participantIDs.length) {
          const r0 = currentApi.sendMessage("✅ اكتملت العملية: تم تغيير كنيات الجميع.", threadID);
          if (r0 && typeof r0.catch === 'function') r0.catch(() => {});
          clearInterval(intervalId);
          global.nickSessions.delete(threadID);
          delete global.nickPersist[threadID];
          saveNickState();

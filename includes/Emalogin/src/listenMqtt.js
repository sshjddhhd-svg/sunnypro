...
function listenMqtt(defaultFuncs, api, ctx, globalCallback) {
        // [FIX] Reconnect deduplication guard — prevents overlapping MQTT clients.
        // Each call gets a generation number. Callbacks from older generations are
        // silently dropped, so a stale client that fires after reconnect won't
        // deliver duplicate events or hold the process open.
        if (!ctx._mqttReconnectGen) ctx._mqttReconnectGen = 0;
        const myGen = ++ctx._mqttReconnectGen;
        const _gc = (err, msg) => { if (ctx._mqttReconnectGen !== myGen) return; globalCallback(err, msg); };
        ...
        mqttClient.on('message', function (topic, message, _packet) {
                ...
                if (jsonMessage.type === "jewel_requests_add") {
                        ...
                        for (const i in jsonMessage.deltas) {
                                ...
                                parseDelta(defaultFuncs, api, ctx, _gc, { "delta": delta });
                        }
                                ...
                                if (delta.deltaMessageReaction && !!ctx.globalOptions.listenEvents) {
                                        ...
                                        const callbackToReturn = {
                                                ...
                                                attachments: (delta.deltaMessageReply.message.attachments || []).map(function (att) {
                                                        try { const mercury = JSON.parse(att.mercuryJSON); Object.assign(att, mercury); } catch (_) {}
                                                        return att;
                                                }).map(att => {
                                                        let x;
                                                        try {

...
/**
 * Converts a Facebook full-permission Access Token (EAAAA...) into
 * session cookies (AppState array).
 * @param {string} tokenFullPermission - Facebook access token starting with EAAAA
 * @returns {Promise<Array>} Array of cookie objects in AppState format
 */
module.exports = async function getFbstate(tokenFullPermission) {
  ...
  throw new Error("No session cookies returned from token");
};
